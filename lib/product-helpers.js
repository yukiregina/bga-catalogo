...
// variant: 'primary' | 'tapa' | { ala: '75' } | { variantLabel: 'Perforada Tipo C' }
export function getProductImageAlt(product, variant = 'primary') {
  ...
  if (variant && typeof variant === 'object' && variant.variantLabel) return `${base} — ${variant.variantLabel}`
  return base
}

// ─── Slug de cor no SKU ──────────────────────────────────────────────────────
// Uppercase, sem acento, não-alfanumérico vira "-", cortado em 22 caracteres
// e sem "-" sobrando na ponta do corte. "BEIGE Texturizado RAL 7032" (28
// caracteres) vira "BEIGE-TEXTURIZADO-RAL" — sem isso duas linhas do mesmo
// SKU em cores diferentes colidem, porque o lineId é o próprio SKU composto.
...
// não reimplementam a lógica. `colorLabel` só se aplica a finish com
// `needsColor` (hoje só pintura electrostática) — nos demais casos vem null.
export function buildComposedSKU(baseSku, axes, materialId, gauge, treatmentId, colorLabel) {
  let sku = baseSku
  ...
  if (gauge) sku += ' · ' + gauge
  if (treatmentId) {
    const names = { pz: 'PZ', gf: 'GF', elec: 'ELEC' }
    sku += ' · ' + (names[treatmentId] ?? treatmentId.toUpperCase())
  }
  if (colorLabel) {
    const slug = slugifyColor(colorLabel)
    if (slug) sku += ' · ' + slug
  }
  return sku
}
...
// config: { variante, axes: { ancho, ala, ... }, material, espesor, finish,
// color } — valores crus (sem o sufixo "(variante a confirmar)" do
// composedSKU). Usado tanto pela ficha (grava ao vivo) quanto pelo carrinho
// (reconstrói o link salvo). `finish` é o eixo novo da seção 6 — convive com
// `material`, que os produtos sem finishes (os outros 71) continuam gravando
// como sempre gravaram.
export function buildConfigQuery(config) {
  ...
  if (config.finish) params.set('finish', config.finish)
...
// { code, attributes } que ST2239 ainda usa. Nenhuma das duas está roteada
// hoje, mas sem isso as opções do <select> renderizariam em branco no dia
// em que entrarem.
...
// `finish` (id de globalSpecs.finishes) tem prioridade sobre `material`: um
// produto com eixo de terminación (seção 6) manda o label do finish; os
// outros continuam resolvendo o nome pelo `material` de sempre.
export function buildConfigLabel({ variant, axes = [], material, gauge, globalSpecs, finish, color } = {}) {
  const parts = []
  ...
  if (finish) {
    const finishLabel = globalSpecs?.finishes?.find(f => f.id === finish)?.label
